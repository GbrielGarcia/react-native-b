...
function TresProcess({ navigation }) {
    ...
    const consul = () => {
        ...
        if (parseInt(number) >= 0) {
            setResult(`${number} * ${number} * ${number} = ${number * number * number}`)
        }else{
            setResult(`Acaba de ingresar un numero negativo ${number}`)
        }
